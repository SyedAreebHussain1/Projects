...
import { addSurveyFormApi } from '../../../../redux/api/Survey'
import MapLocation from './MapLocation'
import { useDispatch } from 'react-redux'
import { errorMessage } from '../../../../utils/message'
const AddSurveyModal = ({ visible, toggle }) => {
    const [form] = useForm()
    const dispatch = useDispatch()

    const [markers, setMarkers] = useState([])
    const [selectedPlace, setSelectedPlace] = useState({})
    const { loading } = useSelector((state) => state?.AddSurveyForm)
    function onFinish(value) {
        ...
        Object.keys(body).length > 13 && addSurveyFormApi(dispatch, body, onSuccess)
    }
    useEffect(() => {
    ...
    return (
        <Modal
            title={<h3 className="text-[18px] font-semibold">Add Survey</h3>}
            open={visible}
            onCancel={onCancel}
            footer={null}
            centered={true}
