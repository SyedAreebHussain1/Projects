...
const Demo = () => {
  ...
  useEffect(() => {
    ...
    articlesFormLocalStorage ? setAllArticles(articlesFormLocalStorage) : ''
  }, [])
    ...
    <section className='mt-16 w-full max-w-xl'>
      ...
      <div className="flex flex-col w-full gap-2">
        ...
        <div className="flex flex-col gap-1 max-h-60 overflow-y-auto">
          {allArticles?.map((item, i) => {
            return <div onClick={() => setArticle(item)} key={`link-${i}`} className="link_card">
              ...
              <p className="flex-1 flex-satoshi text-blue-700 font-medium text-sm truncate">{item?.url}</p>
            </div>
          })}
        </div>
      </div>
      <div className="my-10 max-w-full flex justify-center items-center">
        {isFetching ? (
          ...
          <p className="font-inter font-bold text-black text-center">well, that wasnt supposed to happen... <br /> <span className="font-satoshi font-normal text-gray-700">{error?.data?.error}</span></p>
        ) : article?.summary && (
          <div className="flex flex-col gap-3">
            <h2 className="font-satoshi font-bold text-gray-600 text-xl">Artcle <span className="blue_gradient">Summary</span></h2>
